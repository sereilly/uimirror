...
namespace Microsoft.MixedReality.SpectatorView
{
    ...
    public class SocketerClient
    {
        ...
        /// </summary>
        public event Action<SocketerClient, MessageEvent> Connected;
        /// <summary>
        ...
        /// </summary>
        public event Action<SocketerClient, MessageEvent> Disconnected;
        ...
        {
            Message?.Invoke(this, new MessageEvent() { MessageType = MessageType.Message, SourceHost = Host, Message = e.Data });
        }

        private void TcpServer_Message(object sender, SocketServer.MessageEventArgs e)
        {
            Message?.Invoke(this, new MessageEvent() { MessageType = MessageType.Message, SourceHost = e.RemoteEndpoint.Address.ToString(), Message = e.Data, SourceId = e.SourceId });
        }
        ...
        private void UdpListener_Message(byte[] data, string sourceHost)
        {
            if (SuppressUDPHeaders)
            {
                Message?.Invoke(this, new MessageEvent() { MessageType = MessageType.Message, SourceHost = sourceHost, Message = data });
            }
            else
            {
                // the tcp code already implements the length protocol, the udp code does not.  So implement it here.
                ...
                Message?.Invoke(this, new MessageEvent() { MessageType = MessageType.Message, SourceHost = sourceHost, Message = message });
            }
        }
        ...
        private void TcpClient_Disconnected(object sender, SocketClient.ConnectionEventArgs e)
        {
            ...
            {
                sentDisconnect = true;
                Disconnected?.Invoke(this, new MessageEvent() { MessageType = MessageType.Disconnected, SourceId = 0, SourceHost = Host });
            }
        }
        ...
        private void TcpClient_Connected(object sender, SocketClient.ConnectionEventArgs e)
        {
            ...
            Connected?.Invoke(this, new MessageEvent() { MessageType = MessageType.Connected, SourceId = 0, SourceHost = Host });
        }

        private void TcpServer_Disconnect(object sender, SocketServer.ConnectionEventArgs e)
        {
            Disconnected?.Invoke(this, new MessageEvent() { MessageType = MessageType.Disconnected, SourceId = e.SourceId, SourceHost = e.RemoteAddress.Address.ToString() });
        }

        private void TcpServer_Connect(object sender, SocketServer.ConnectionEventArgs e)
        {
            Connected?.Invoke(this, new MessageEvent() { MessageType = MessageType.Connected, SourceId = e.SourceId, SourceHost = e.RemoteAddress.Address.ToString() });
        }
...
#region Private TCP Implementation
        ...
        private class SocketClient
        {
            ...
            private CommandData ReceiveBuffer()
            {
                try
                {
                    NetworkStream ns = reader.BaseStream as NetworkStream;
        ...
        private class SocketServer
        {
...
#else
            private static bool ReceiveAll(Socket socket, byte[] buffer, int offset, int count)
            {
                try
                {
                    int pos = 0;
            ...
            private static bool ReceiveAll(Socket socket, byte[] buffer)
            {
                return ReceiveAll(socket, buffer, 0, buffer.Length);
            }
